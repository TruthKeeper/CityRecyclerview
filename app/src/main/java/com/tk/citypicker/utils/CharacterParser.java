...
public class CharacterParser {
    ...
    public static final String[] PY_STR = new String[]{"a", "ai", "an", "ang", "ao",
    ...
    //多音字手动添加
    private static final String[] POLYPHONE = new String[]{
            "重庆,chong", "长沙,chang", "长春,chang", "长白山,chang"};
     ...
     * @return A-Z，#
     */
    public static final char convert(String str) {
        char result = '#';
        if (TextUtils.isEmpty(str)) {
            return result;
        }
        // 多音字判定
        for (String polyphone : POLYPHONE) {
            String[] chinese = polyphone.split(",");
            if (str.equals(chinese[0])) {
                // 是多音字
                return chinese[1].toUpperCase().charAt(0);
            }
        }
        int ascii = getChsAscii(str.substring(0, 1));
        if (ascii > 0 && ascii < 160) {
            return result;
        } else {
            for (int i = (PY_VALUE.length - 1); i >= 0; i--) {
                if (PY_VALUE[i] <= ascii) {
                    return PY_STR[i].toUpperCase().charAt(0);
                }
            }
        }
        return result;
